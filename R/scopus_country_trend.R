...
scopus_country_trend <- function( res ) {
  ...
  ggplot2::ggplot() +
    ggplot2::geom_bar( data = data.tib, ggplot2::aes( x = month ) )
  ...
  mag <- max(monthly_data.tib |> dplyr::select(n)) /
         max(yearly_data.tib |> dplyr::select(n)) - 0.2
  ...
  g_line <- yearly_data.tib |>
    ggplot2::ggplot(
      ...
      ggplot2::geom_area(
        ...
        ggplot2::aes( x = as.Date(paste0(month,"-01")), y = n/mag, group = NA, colour = NA ),
        show.legend = FALSE ) +
      ...
      ggrepel::geom_label_repel(
        ...
        hjust = 0, nudge_x = 60, direction = "y", seed = 42,
        segment.color = NA, show.legend = FALSE ) +
      ggrepel::geom_text_repel(
        ggplot2::aes( label = n ), vjust = -1, nudge_x = -10,
